...
alter table public.edited_video_comments
  drop constraint if exists edited_video_comments_content_check;

alter table public.edited_video_comments
  drop constraint if exists edited_video_comments_comment_type_check;

alter table public.edited_video_comments
  add constraint edited_video_comments_comment_type_check
  check (comment_type in ('text', 'voice', 'image'));

alter table public.edited_video_comments
  add constraint edited_video_comments_content_check
  check (
    ...
    or (
      ...
      and nullif(trim(image_storage_path), '') is not null
    )
  );
...
create or replace function public.client_add_edited_video_comment(
  ...
  comment_image_storage_path text default null
)
returns uuid
language plpgsql
security definer
...
begin
  if next_comment_type not in ('text', 'voice', 'image') then
  ...
  clean_image_path := nullif(trim(coalesce(comment_image_storage_path, '')), '');
  ...
  if next_comment_type = 'image' and clean_image_path is null then
    raise exception 'Comment image is required';
  end if;
...
create or replace function public.client_review_edited_video(
  ...
  review_image_storage_path text default null
)
returns void
language plpgsql
security definer
...
begin
  ...
  if clean_comment_type not in ('text', 'voice', 'image') then
    raise exception 'Invalid comment type';
  end if;

  clean_body := nullif(trim(coalesce(review_comment, '')), '');
  clean_audio_path := nullif(trim(coalesce(review_audio_storage_path, '')), '');
  clean_image_path := nullif(trim(coalesce(review_image_storage_path, '')), '');
  ...
  elsif clean_comment_type = 'image' and clean_image_path is not null then
    perform public.client_add_edited_video_comment(
      target_edited_video_id,
      'image',
      null,
          ...
          'comments', coalesce((
            select jsonb_agg(
              jsonb_build_object(
                ...
                'image_storage_path', c.image_storage_path,
...
drop policy if exists "Upload comment images" on storage.objects;
create policy "Upload comment images"
  on storage.objects for insert
  to anon, authenticated
  with check (
    bucket_id = 'uploads'
    and (storage.foldername(name))[1] = 'comment-images'
  );

drop policy if exists "Read comment images" on storage.objects;
create policy "Read comment images"
...
grant execute on function public.client_add_edited_video_comment(uuid, text, text, text, integer, text) to authenticated;
grant execute on function public.editor_add_edited_video_comment(text, uuid, text, text, text, integer, text) to anon, authenticated;
grant execute on function public.admin_add_edited_video_comment(uuid, text, text, text, integer, text) to authenticated;
grant execute on function public.client_review_edited_video(uuid, text, text, text, text, integer, text) to authenticated;
